...
def _get_vendor_from_mac(mac_prefix: str) -> str:
    ...
    """
    if mac_prefix in _MAC_VENDORS:
        return f" ({_MAC_VENDORS[mac_prefix]})"
    return ""
...
class MacScanner:  # pylint: disable=too-few-public-methods
    ...
    def get_mac_info(self, ip: str) -> str:
        ...
        # We extract the 8-char prefix (XX:XX:XX) to maximize cache hit rate.
        # Passing full MAC would cause cache misses for every unique device.
        prefix = mac[:8].upper()
        vendor = _get_vendor_from_mac(prefix)
        return f"[{mac}]{vendor}"
